...
age/gender and hobby names. The person can be skipped with **下一位**.
...
largest qualifying score, or a reason if no candidate qualifies.
...
3. Run the Flutter app from `near-ride/app` and open **推薦**. Enable participation
   on **both accounts**.
4. Confirm the screen displays one person, **下一位** skips that person, and
