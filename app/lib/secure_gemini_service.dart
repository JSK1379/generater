...
/// Gemini client for Near Ride.
///
/// The mobile app never stores or sends a Gemini API key. All model access is
/// proxied through the Near Ride backend, where GEMINI_API_KEY is configured as
/// a server environment variable.
class SecureGeminiService {
  final Map<String, String> _aiPersonalities = const {
    'default': '友善、簡潔',
    'funny': '幽默風趣',
    'professional': '專業正式',
    'casual': '輕鬆自然',
      ...
      final response = await http
          .post(
            ...
            body: jsonEncode({
              ...
              'context': context,
              'roomId': roomId,
              'personality': includePersonality ? _currentPersonality : 'default',
            }),
          )
  ...
  List<String> getAvailablePersonalities() => _aiPersonalities.keys.toList();

  String get currentPersonality => _currentPersonality;

  String getPersonalityDescription(String personality) =>
      _aiPersonalities[personality] ?? _aiPersonalities['default']!;

  // Compatibility methods for the old API-key setup UI. Keys are intentionally
  // no longer accepted or stored on the device.
  Future<bool> setApiKey(String apiKey) async {
    debugPrint('[AI] API keys are server-managed; local key ignored');
    return false;
  }
